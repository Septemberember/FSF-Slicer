...
from .pipeline import analyze, slice_only, verify_only, runtime_metadata
...
def build_parser() -> argparse.ArgumentParser:
    ...
    analyze_cmd.add_argument("--force", action="store_true", help="Write an inconclusive report for invalid FSF input")
    ...
    benchmark = commands.add_parser('benchmark', help='Run paired repetitions and program-cluster confidence intervals')
    ...
    benchmark.add_argument('--modes', default='fsf', help='Comma-separated fsf,backward,conditioned')
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "analyze":
            ...
            if result.get('original_compilation', {}).get('ok') is False or any(s.get('compile_ok') is False for s in result.get('slices', {}).values()):
                return 2
        elif args.command == 'benchmark':
            from .benchmark import run_benchmark
            result = run_benchmark(args.manifest, args.output, args.repeats, args.seed, tuple(args.modes.split(',')))
            print(json.dumps({'rows': result['rows'], 'failures': result['failures'], 'summary': str(Path(args.output)/'summary.json')}, indent=2))
            return 2 if result['failures'] else 0
        elif args.command == "slice":
            results = slice_only(args.java, args.fsf, args.output)
            print(json.dumps(results, ensure_ascii=False, indent=2))
            return 2 if any(s['compile_ok'] is False for s in results) else 0
        elif args.command == "verify":
            print(json.dumps(verify_only(args.java, args.fsf), ensure_ascii=False, indent=2))
            ...
            print(json.dumps({
                ...
                "ready": runtime['javac'].startswith('javac '),
                "environment": runtime,
            }, indent=2))
        return 0
...
def _init_fsf(java: str, method: str | None, output: str) -> None:
    ...
    if program.return_type == 'void':
        raise FSFToolError('Void methods require an FSF file with explicitly selected exported locals.')
    data = {
        "method": program.method_name,
        "inputs": {name: {"type": kind, **({'min': -100, 'max': 100} if kind in {'int', 'long'} else {})} for name, kind in program.parameters.items()},
...
def _dataset_check(folder: str) -> dict[str, object]:
    root = Path(folder)
    if not root.is_dir():
        raise FSFToolError(f'Java dataset directory does not exist: {root}')
    ...
    for path in files:
        try:
            ...
            successes.append({"file": str(path), "class": program.class_name, "method": program.method_name, "inputs": program.parameters, "return": program.return_type, "semantic_support_issues": support_issues(program)})
