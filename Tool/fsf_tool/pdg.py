...
class ProgramDependenceGraph:
    ...
    cfg: Any = None
    data_edges: set[tuple[int, int]] = field(default_factory=set)
    control_edges: set[tuple[int, int]] = field(default_factory=set)
...
class PDGBuilder:
    ...
    def build(self) -> ProgramDependenceGraph:
        ...
        cfg = ControlFlowGraph(self.program, self.nodes, self.node_ids, self.parameter_nodes)
        data_edges = cfg.data_edges()
        control_edges = cfg.control_edges()
        for node in self.nodes.values():
            for parent in node.control_parents:
                control_edges.add((parent, node.id))
        for source, target in data_edges | control_edges:
            self._edge(source, target, edges, reverse)
