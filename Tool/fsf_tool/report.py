...
def slice_to_dict(result: SliceResult) -> dict[str, Any]:
    return {
        ...
        "warnings": result.warnings,
        "core_nodes": sorted(result.core_node_ids),
        "pruning_proofs": result.pruning_proofs,
        "elapsed_ms": round(result.elapsed_ms, 3),
    }
...
def render_html(payload: dict[str, Any]) -> str:
    title = html.escape(Path(payload.get("program", "FSF-TBFV report")).name)
    ...
    setup = html.escape(json.dumps({'analysis_domain': payload.get('analysis_domain'), 'configuration': payload.get('configuration'), 'environment': payload.get('environment'), 'source_sha256': payload.get('source_sha256'), 'fsf_sha256': payload.get('fsf_sha256')}, indent=2, ensure_ascii=False))
    for scenario_id, result in sliced_results.items():
        ...
        comparison = payload.get('comparison', {}).get(scenario_id, {})
        preservation = comparison.get('judgment_agreement', 'not_compared')
        behavior = comparison.get('behavior', {}).get('status', 'not_checked')
        scenario_sections.append(
            ...
            f"<p><b>Slice:</b> LOC {original_metrics.get('loc','?')} → {metrics.get('loc','?')}, statements {original_metrics.get('executable_statements','?')} → {metrics.get('executable_statements','?')}, complexity {original_metrics.get('cyclomatic_complexity','?')} → {metrics.get('cyclomatic_complexity','?')}; compile={_e(slice_info.get('compile_ok'))}; judgment agreement={_e(preservation)}; behavior={_e(behavior)}</p>"
            f"<p><b>Diagnostics:</b> {_e('; '.join(result.get('warnings', []) + slice_info.get('warnings', [])))}</p>"
            f"<pre>{_e(json.dumps({'soundness_counterexample': result['soundness'].get('counterexample'), 'completeness_counterexample': result['completeness'].get('counterexample'), 'behavior': comparison.get('behavior')}, indent=2))}</pre>"
